...
from ANN import network_for_features
...
def for_evolution(net,train_loader,format = 'train'):
  """ 进化计算对接函数
  :param net: 训练的特征提取网络
  :param train_loader: 训练数据集
  :param format: 选择模型是训练还是测试
  ...
  for epoch in range(1):
      ...
      for X, y in train_loader:
        # 若是scrathing方式，则 output = X.view(X.size(0),-1).detach().numpy()
        output = net.query(X.view(X.size(0),-1))
        som = SOM(output, (1, 10), 100, 3000)
        som.train()
        res = som.train_result()
        transform = {}
...
if __name__ == "__main__":
  ...
  try:
    net.load_state_dict(torch.load("trained_model/feature_24.pth"))
  except:
    pass
